...
const Profile = () => {
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      ...
      confirmpass: yup.string().required("Confirm new password")
    }),
    ...
    <section className="h-screen lg:ms-94 lg:mt-30">
      <div className="flex gap-5">
        ...
        <div className="w-24 h-24 flex justify-center items-center rounded-full border-2 border-blue-hover font-semibold text-white bg-blue-sapphire">
          <h1 className="text-5xl">TA</h1>
        </div>
         {/* Full name & Email */}
        <div className="flex flex-col justify-center font-medium">
          <h1 className="text-2xl">Taiwo Adetunji</h1>
          <p className="text-sm">dontiwo5020@gmail.com</p>
        </div>
      </div>
        ...
        <div className="mt-20">
          <form autoComplete="off" method="POST" className="w-full lg:w-2/5 p-5" onSubmit={formik.handleSubmit} >
            <div className="flex flex-col mb-5">
              <label htmlFor="currentpass" className="ml-1">Current password</label>
              <input type="password" id="currentpass" name="currentpass" 
              placeholder="Enter your current password"
              className="px-4 py-2 my-2 border rounded-2xl"
              onChange={formik.handleChange}
              onBlur={formik.handleBlur}
              value={formik.values.currentpass}
              />
              <small className="text-red-600">{formik.touched.currentpass && formik.errors.currentpass}</small>
            </div>
            <div className="flex flex-col mb-5">
              <label htmlFor="newpass" className="ml-1">New password</label>
              ...
              />
              <small className="text-red-600">{formik.touched.newpass && formik.errors.newpass}</small>
            </div>
            <div className="flex flex-col mb-10">
              <label htmlFor="confirmpass" className="ml-1">Confirm password</label>
              ...
              />
              <small className="text-red-600">{formik.touched.confirmpass && formik.errors.confirmpass}</small>
            </div>
            <div>
